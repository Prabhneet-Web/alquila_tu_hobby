...
class ScalingUtils {
  ...
  void setCurrentDeviceSize(BuildContext context) {
    ...
    if (!kIsWeb &&
      ...
      currentDeviceSize = Size(currentDeviceSize.width,
          (figmaPhonePort.height / figmaPhonePort.width * currentDeviceSize.width));
    }
  ...
  EdgeInsetsGeometry getPadding(
    ...
    return EdgeInsets.only(
      left: kIsWeb ? left ?? 0 : getScaledWidth(
        left ?? 0,
      ),
      top: kIsWeb ? top ?? 0 : getScaledHeight(
        top ?? 0,
      ),
      right: kIsWeb ? right ?? 0 : getScaledWidth(
        right ?? 0,
      ),
      bottom: kIsWeb ? bottom ?? 0 : getScaledHeight(
        bottom ?? 0,
      ),
    );
  }
  ...
  double get heightRatio =>
      !kIsWeb ? currentDeviceSize.height / (figmaPhonePort.height) : fh / (desktopViewPort.height);
  double get widthRatio =>
      !kIsWeb ? currentDeviceSize.width / (figmaPhonePort.width) : fw / (desktopViewPort.width);
  ...
  double getScaledFont(num value) => (value).floorToDouble();
}

class ScalingUtility {
  static final ScalingUtils _scalingUtility =
  ScalingUtils()..setCurrentDeviceSize(Get.context!);
  ...
  double getScaledFont(num value) => _scalingUtility.getScaledFont(kIsWeb ? value : fontRatio * value);
}
